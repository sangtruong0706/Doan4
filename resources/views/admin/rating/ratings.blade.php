        ...
        <div class="container-fluid">
            ...
            <div class="card">
                ...
                <form action="" method="get">
                    <div class="card-header">
                        <div class="card-title">
                            ...
                            <button type="button" onclick="window.location.href=ratingIndexRoute" class="btn btn-default btn-sm">Reset</button>
                        </div>
                                ...
                                @foreach ($ratings as $rating)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <a href="#" onclick="deleteCategory({{ $rating->id }})" class="text-danger w-4 h-4 mr-1">
                ...
                <div class="card-footer clearfix">
                    {{ $ratings->links('vendor.pagination.custom_admin') }}
                </div>
            </div>
        </div>
        <!-- /.card -->
...
<script>
    var ratingIndexRoute = "{{ route('ratings.productRating') }}";
    ...
    function changeStatus(status, id) {
        Swal.fire({
        title: 'Are you sure?',
        text: 'You want to change status rating?',
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        confirmButtonText: 'Yes, delete it!'
            ...
            if (result.isConfirmed) {
                // Thực hiện ajax để xóa category
                $.ajax({
                    url: '{{ route("ratings.changeRatingStatus") }}',
                    type: 'get',
                    data: {
                        status: status,
                        id: id,
                    ...
                    success: function(response){
                        if (response.status == true) {
                            window.location.href = '{{ route("ratings.productRating") }}'
                        }
                    },
                    error: function(jqXHR, exception) {
                        console.log("Something went wrong!")
